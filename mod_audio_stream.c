                                     ...
                                     char *metadata)
{
    ...
    switch_vad_set_param(s_vad->svad, "debug", 1);
    ...
    if ((status = switch_core_media_bug_add(session, MY_BUG_NAME, NULL, capture_callback, s_vad, 0, flags, &bug)) != SWITCH_STATUS_SUCCESS)
    {
        return status;
    }
